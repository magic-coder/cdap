...
public class DefaultStoreTest {
  ...
  public void testDynamicScheduling() throws Exception {
    ...
    store.deleteSchedule(Id.Schedule.from(program, "Schedule2"));
    schedules = getSchedules(appId);
    Assert.assertEquals(1, schedules.size());
  ...
  public void testAdapterMDSOperations() throws Exception {
    ...
    AdapterSpecification retrievedAdapter = store.getAdapter(Id.Adapter.from(namespaceId, "nonExistingAdapter"));
    Assert.assertNull(retrievedAdapter);

    //Retrieve specs
    AdapterSpecification retrievedSpec = store.getAdapter(Id.Adapter.from(namespaceId, "streamToAvro1"));
    Assert.assertEquals(specStreamToAvro1, retrievedSpec);
    // Remove spec
    store.removeAdapter(Id.Adapter.from(namespaceId, "streamToAvro1"));

    // verify the deleted spec is gone.
    retrievedAdapter = store.getAdapter(Id.Adapter.from(namespaceId, "streamToAvro1"));
    Assert.assertNull(retrievedAdapter);

    // verify the other adapter still exists
    retrievedSpec = store.getAdapter(Id.Adapter.from(namespaceId, "streamToAvro2"));
    ...
    retrievedAdapter = store.getAdapter(Id.Adapter.from(namespaceId, "streamToAvro2"));
    Assert.assertNull(retrievedAdapter);
  }
}
